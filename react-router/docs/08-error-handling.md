...
NuxtのエラーハンドリングとReact Routerの実装を比較してみましょう：
...
|------|---------|--------------|
| グローバルエラー処理 | `error.vue` ページ | ルートレベル `errorElement` |
| ページレベルエラー | `createError()` | `throw Response()` |
| エラーページ | `error.vue` | `errorElement` |
| エラー情報取得 | `clearError()` | `useRouteError()` |
| 404ページ | `[...slug].vue` | `path: "*"` |
| エラー回復 | `clearError()` | カスタム実装 |
| ローダーエラー | `asyncData` throw | `loader` throw Response |
